...
do
    ...
    if [ -a $infile ]
    then
        echo SCRIPT: Removing $infile
        rm $infile
    fi

    if [ -a $outfile ]
    then
        echo SCRIPT: Removing $outfile
        rm $outfile
    fi
    ...
    cat sample_data > $infile
    echo "$(dd if=/dev/urandom bs=1 count=1000000)" >> $infile

    # Start server
    echo SCRIPT: Starting server with port:$sport file:$outfile p:$p
    # ex. python server.py 7735 outfile .2 &
    ...
    # ex. python client.py 10.0.0.3 7735 infile 4 1200 &
